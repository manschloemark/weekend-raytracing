...
/*
 A note on the rotations, because it's pretty clever

 Much like for translations, we are manipulating the ray instead of the
 actual object.

 So instead of rotating the shape in space, the ray is being rotated.
 If the rotated ray hits the object, you must re-rotate the
 hit_record's point and normal. This is because the point you hit on the hit
 record is the un-rotated object's point, but you want to tell the main
 program where the point would be if the object was actually rotated.
 So you rotate the point in the opposite direction of the ray.
*/
